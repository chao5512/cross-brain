...
@Api(value = "模型管理",description = "参数说明:modelType:1-机器学习,2-深度学习;日期格式:yyyyMMdd")
public class ModelController {
    ...
    @ResponseBody
    public Result findModelByModelId(@RequestParam(name="modelId") String modelId,
                                HttpServletResponse response){
        Result result = null;
    ...
    @ApiOperation(value = "按时间范围和类型查询模型",httpMethod = "POST")
    @RequestMapping(value= "/findByTypeAndTime",method = RequestMethod.POST)
    @ResponseBody
    public Result findByTypeAndTime(@RequestParam(name="startDate")String startDate,
                                               @RequestParam(name="endDate")String endDate,
                                               @RequestParam(name="modelType")short modelType,
                                               @RequestParam(name="owner") String owner){
        Result result = null;
        List<Model> models = modelService.findByTimeAndType(startDate,endDate,modelType,owner);
        result = ResultUtil.success(models);
        return result;
    }
